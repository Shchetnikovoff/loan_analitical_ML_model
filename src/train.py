...
def main():
    ...
    if not os.path.exists(TRAIN_PATH) or not os.path.exists(TEST_PATH):
        print(f"Error: Data files not found in {DATA_DIR}. Please download credit risk data files.")
        return
    ...
    # 3. Modeling (Case 9)
    print("Training credit risk model (Gradient Boosting with GridSearchCV)...")
    best_model, best_params = train_model(X_train_processed, y_train)
    print(f"Best Parameters: {best_params}")

    # 4. Evaluation
    print("Evaluating credit risk model...")
    evaluate_model(best_model, X_val_processed, y_val, label_encoder)

    # 5. Submission
    print("Generating credit risk submission...")
    ...
    print(f"Credit risk submission saved to {SUBMISSION_PATH}")
